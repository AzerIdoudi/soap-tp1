...
Object.keys(client.CalculatorService.CalculatorPort));
 ...
 // Test Modulo 
 const modResult = await client.ModuloAsync({ a: 30, b: 4 });
 console.log(`Modulo: 30 % 4 = ${modResult[0].result}`);
 // Test Puissance
 const powResult = await client.PowerAsync({ a: 4, b: 8 });
 console.log(`Puissance: 4 ^ 8 = ${powResult[0].result}`);
 // Test TemperatureService
 const tempResult = await client.TemperatureServiceAsync({ celsius: 25 });
 console.log(`TemperatureService: 25°C = ${tempResult[0].result}°F`);
 // Test Division par zéro (erreur)
 console.log('\n--- Test erreur: Division par zéro ---');
 try {
 await client.DivideAsync({ a: 10, b: 0 });
 } catch (error) {
 console.log('❌ Erreur capturée:', error.root?.Envelope?.Body?.Fault?.Reason?.Text || error.message);
 }
 } catch (error) {
